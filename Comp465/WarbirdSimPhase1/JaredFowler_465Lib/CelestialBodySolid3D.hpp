...
private:
    ...
    glm::mat4 lastModelMatrix;    //Stores last model matrix so child obj doesn't have to recalculate it.
...
public:
    ...
    CelestialBodySolid3D(char* objName, char* fileName, UINT32 vertexCount, float modelSize, bool active = true, 
        ...
        : Object3D(objName, fileName, vertexCount, modelSize, active, scaleMatrix,
                   translationMatrix, rotationalMatrix)
    {
        ...
        //We'll also want to initialize our lastModelMatrix to the identity
        setLastModelMatrix(glm::mat4());
    }
    ...
    inline bool setMissleCount(UINT32 m){ this->missleCount = m; return true; }
...
glm::mat4 CelestialBodySolid3D::getModelMatrix(void){
    ...
    if (this->orbital)
        returnM *= glm::rotate(glm::mat4(), this->currentRotation, this->rotationAxis);
    ...
    if (this->spinning)
        returnM *= glm::rotate(glm::mat4(), this->currentSpin, this->spinAxis);
