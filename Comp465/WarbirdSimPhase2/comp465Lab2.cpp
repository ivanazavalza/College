...
void init(void) {
    ...
    //Get handles for the uniform structures in the shader programs
    ...
    //We can set some of these uniform values which wont change
    //glUniform1f(PLR, 2000.0f);
...
void display(void){
    ...
    glUniform4fv(PLL, 1, glm::value_ptr(viewMatrix * celestialSolid[ruber_mod]->getLastModelMatrix() * glm::vec4(0, 0, 0, 1)));

    //Get Light Locations and send to shaders
    //glUniform4fv(PLL, 1, glm::value_ptr(viewMatrix * celestialSolid[ruber_mod]->getLastModelMatrix() * glm::vec4(0, 0, 0, 1)));
    //glUniform1f(PLR, 2000.0f);
