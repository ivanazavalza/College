...
/*Loads data from *.tri model file into a vao's vbo buffer for vertex, color, normal values and
returns bounding radius of model. Originally written by Mike Barnes. Modified by Jared Fowler.*/

float loadModelBuffer(char modelFile[25], GLuint nVertices, GLuint vao, GLuint vbo, GLuint shaderProgram,
                      char * shaderVertex, char * shaderColor, char * shaderNormal)
{
    ...
    //Did an error occur while getting the model information?
    ...
    /*Set up vertex shader variable handles*/
