...
namespace AGMGSKv6
{
    public class EyeCandy_MoonBaseAlpha : Model3D
    {
        //Variables---------------------------------------------------------------------------------------------
        //Void

        //Constructor-------------------------------------------------------------------------------------------
        public EyeCandy_MoonBaseAlpha(Stage theStage, string label, string meshFile, bool isCollidable = true)
            : base(theStage, label, meshFile, isCollidable)
        {
            ...
            for (UInt32 i = 0; i < location.GetLength(0); i++)
            {
                ...
                addObject(new Vector3(xPos * spacing, terrain.surfaceHeight((int)xPos, (int)zPos), zPos * spacing), Vector3.Up, 0.0f, new Vector3(15,15,15));
            }
        }
    }
}