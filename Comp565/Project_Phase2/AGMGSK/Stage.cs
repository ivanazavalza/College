 ...
 *      -Added a print statement which prints out the curent packing percentage.
 * 
 */
...
namespace AGMGSKv6
{
    ...
    /// </summary>
    public partial class Stage : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            if (npAgent.treasure_count + player.treasure_count >= 5)
            {
                if (npAgent.treasure_count < player.treasure_count)
                {
                    inspector.setInfo(17, string.Format("Treasures Collected:  NPAgent: {0}   Player: {1}     PLAYER WON!", npAgent.treasure_count, player.treasure_count));
                }
                else
                {
                    inspector.setInfo(17, string.Format("Treasures Collected:  NPAgent: {0}   Player: {1}    NPAGENT WON!", npAgent.treasure_count, player.treasure_count));
                }
            }
            else
            {
                inspector.setInfo(17, string.Format("Treasures Collected:  NPAgent: {0}   Player: {1}", npAgent.treasure_count, player.treasure_count));
            }

            //Display information concerning packing
            inspector.setInfo(19, string.Format("Packing Probability: {0}%", this.pack.PackingProbability));
